...
class LoginController extends Controller
{
    ...
    public function login(Request $request)//: RedirectResponse
    {
        ...
        $user = $res['data'];
        // dd($res['status']);
        // Cek apakah user dan pass benar
        if ($res['status'] !== '200') {
        ...
        // Cek apakah salah satu elemen dalam array 'detil' memiliki 'apps_id' yang sesuai
        $isValidAppId = false;
        foreach ($user['detil'] as $detail) {
            if ($detail['apps_id'] === env('APP_ID', 'APP003')) {
                $isValidAppId = true;
                $roleInstance = $detail;
                break;
            }
