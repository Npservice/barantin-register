...
class RegisterController extends Controller
{
    ...
    public function StoreRegisterPerorangan(RegisterRequesPerorangantStore $request, string $id)
    {
        ...
        if ($dokumen->contains('KTP') || $dokumen->contains('PASSPORT')) {
            ...
            $lingkupAktivitas = array_map(function ($activityId) use ($activityMapping) {
                return [
                    'id' => $activityId,
                    'activity' => $activityMapping[$activityId] ?? null
                ];
            }, $request->lingkup_aktivitas);
            
            $jsonLingkupAktivitas = json_encode($lingkupAktivitas);
            
            // Add the processed data to the request
            $requestData = $request->all();
            $requestData['lingkup_aktivitas'] = $jsonLingkupAktivitas;
    ...
    public function StoreRegisterPerusahaan(RegisterRequestPerusahaanStore $request, string $id)
    {
        ...
        // Process the 'lingkup_aktivitas' data
        $activityMapping = [
            1 => 'Import',
            2 => 'Domestik Masuk',
            3 => 'Export',
    ...
    public static function inputRender(Request $request, string $preRegisterId)
    {
        $request->merge([
            ...
            'lingkup_aktifitas' => $request->lingkup_aktivitas,
        ]);
     ...
     */
    public function CheckRegister(mixed $preRegister, bool $isPrint = false): RedirectResponse|bool
    {
        ...
        /* ambil data terbaru untuk pengecekan bahwa status sudah fix */
        $register = Register::where('pre_register_id', $preRegister->id)->orderBy('updated_at', 'DESC')->first();

        if (isset($register) && !$isPrint) {
            if ($register->status == 'MENUNGGU' || $register->status == 'DISETUJUI') {
                abort(redirect()->route('register.message')->with(['message_waiting' => 'Data sedang di proses upt yang dipilih atau yang terdaftar sebelumnya', 'id' => $preRegister->id]));
            }
        }
    ...
    {
        $register = PreRegister::find($id);
        $this->CheckRegister($register, true);

        // Mengambil data dari PjBarantin berdasarkan pre_register_id
        $barantin = PjBarantin::where('pre_register_id', $id)->first();
        $masterUptIds = Register::where('pre_register_id', $id)->pluck('master_upt_id');
        $dokumenPendukung = DokumenPendukung::where('pj_barantin_id', $barantin->id)
            ->select('jenis_dokumen', 'nomer_dokumen', 'tanggal_terbit')
            ->get();
        // Initialize an empty array to hold the UPT information
        $uptInfo = [];

        // Iterate over each master_upt_id and get the corresponding UPT details
        foreach ($masterUptIds as $masterUptId) {
            $upt = BarantinApiHelper::getMasterUptByID($masterUptId);
            $uptInfo[] = $upt['nama_satpel'] . ' - ' . $upt['nama'];
        }

        // $combinedUptInfo = implode(', ', $uptInfo);

        $barantin['upt'] = $uptInfo;
        $barantin['dokumen_pendukung'] = $dokumenPendukung;
        $barantin['daftar_komoditas'] = json_decode($barantin->daftar_komoditas, true);
        // Generate QR code
        ...
        $qrCode = (new QRCode($options))->render(route('register.print', ['id' => $barantin->pre_register_id]));


        // Mengambil data yang dibutuhkan untuk PDF
        $data = [
            'barantin' => $barantin,
            'qrCode' => $qrCode,
        ];
