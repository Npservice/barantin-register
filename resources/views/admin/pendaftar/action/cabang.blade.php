@if ($model->baratinCabang->user_id)
    @if ($model->blockir)
        <a class="btn btn-outline-success btn-sm me-2"
            onclick="Open('{{ route('admin.pendaftar.open.akses', $model->id) }}', '{{ $model->baratinCabang->nama_perusahaan }}')"><i
                class="fas fa-user-check"></i></a>
    @endif
    @if (!$model->blockir)
        <a class="btn btn-outline-danger btn-sm me-2"
            onclick="Block('{{ route('admin.pendaftar.block.akses', $model->id) }}', '{{ $model->baratinCabang->nama_perusahaan }}')"><i
...
@if (!auth()->guard('admin')->user()->upt_id)
    @if (!$model->baratinCabang->user_id)
        <a class="btn btn-outline-warning btn-sm ms-2"
            onclick="CreateUser('{{ route('admin.pendaftar.create.user', $model->id) }}','{{ $model->baratinCabang->nama_perusahaan }}')">
            <i class="fas fa-user-edit"></i></a>
    @endif
    @if ($model->baratinCabang->user_id)
        <a class="btn btn-outline-primary btn-sm ms-2"
            onclick="UserSetting('{{ route('admin.pendaftar.send.user', $model->baratinCabang->user_id) }}','{{ $model->baratinCabang->nama_perusahaan }}')">
